...
import { ORDER_TYPE, ORDER_STATUS } from "@/interfaces/const/order.const";
import { ChargeInvoiceDTO, CreateInvoiceDTO } from "@/interfaces/invoice";
import InvoicesSurcharges from "@/main/models/Invoice/InvoiceSurchargeModel";
import { generateNextInvoiceNumber } from "@/main/utils/invoiceUtils";
import { createSaleMovement } from "./cashService";
...
export async function createOrder(
  ...
  try {
    ...
    const metadataPayload = {
      ...
      notesOrder: orderData.notes,
      userId: orderData.userId,
    };

    await OrderMetadata.create(metadataPayload, { transaction });
    ...
    const _orders = await Order.findAll({
      ...
      include: [
        ...
        },
        {
          ...
          as: "metadataOrder",
          attributes: {
            exclude: ["orderId", "status", "userId", "createdAt", "updatedAt"],
          },
        },
        {
          model: OrderDetail,
      ...
      order: [
        ["createdAt", "DESC"],
        [{ model: OrderDetail, as: "items" }, "sequence", "ASC"],
      ],
    });
...
export async function UpdateQuantityItemsOrder(
  ...
  try {
    ...
    if (configStatus?.paramStatus === ORDER_STATUS.CANCELED) {
      await transaction.rollback();
...
export async function cancelOrder(
  ...
  try {
    ...
    const configCancelItem = await ItemOrdStatus.findOne({
      where: { paramStatus: ORDER_STATUS.CANCELED },
      transaction,
    });
    const configCancelOrder = await OrderStatus.findOne({
      where: { paramStatus: ORDER_STATUS.CANCELED },
...
export async function invoiceOrder(
  dataInvoice: CreateInvoiceDTO,
): Promise<RESPONSE<OrderDTO>> {
  const transaction = await sequelize.transaction();
  try {
    console.log("invoiceOrder called with data:", dataInvoice);
    
    // 1. Buscar la orden
    const orderIdStr = String(dataInvoice.orderId);
    console.log("Looking for order with id:", orderIdStr);
    const order = await Order.findByPk(orderIdStr, { transaction });
    if (!order) throw new Error("Orden no encontrada");

    //Validar si es factura electronica
    if (dataInvoice.electronicInvoice && dataInvoice.customerId) {
      order.customerId = dataInvoice.customerId;
    }

    // 1.5 Validar caja abierta - OBLIGATORIO para toda venta
    let cashRegisterId: string;
    
    if (dataInvoice.cashRegisterId) {
      const cashRegister = await CashRegister.findOne({
        where: { id: dataInvoice.cashRegisterId, isOpen: true },
        transaction,
      });
      if (!cashRegister) {
        throw new Error("La caja está cerrada. Debe abrir una caja antes de registrar ventas.");
      }
      cashRegisterId = dataInvoice.cashRegisterId;
    } else {
      const openCashRegister = await CashRegister.findOne({
        where: { userId: dataInvoice.userId, isOpen: true },
        transaction,
      });
      if (!openCashRegister) {
        throw new Error("No hay caja abierta. Debe abrir una caja antes de registrar ventas.");
      }
      cashRegisterId = String(openCashRegister.id);
    }

    // 2. Buscar status cancelado
    const configCancelItem = await ItemOrdStatus.findOne({
      where: { sequence: 0, paramStatus: "CANCELED" },
      transaction,
    });
    if (!configCancelItem) throw new Error("Config cancel item no encontrado");

    // 3. Traer items de la orden no cancelados y si informacion para la factura
    const orderDetails = await OrderDetail.findAll({
      where: {
        orderId: dataInvoice.orderId,
        status: { [Op.ne]: configCancelItem.id },
      },
      include: [
        {
          model: MenuItem,
    ...
    // 4. Calcular subtotal y total -- Verificar que sean el mismo que se muestra en pantalla
    ...
    const configInvoiceOrder = await OrderStatus.findOne({
      where: { paramStatus: ORDER_STATUS.INVOICED },
      transaction,
    });
    if (!configInvoiceOrder) throw new Error("Estado INVOICED no encontrado");

    // 5.1 Buscar estado INVOICED para items
    const configInvoiceItem = await ItemOrdStatus.findOne({
      where: { paramStatus: ORDER_STATUS.INVOICED },
      transaction,
    });
    if (!configInvoiceItem) throw new Error("Estado INVOICED para items no encontrado");

    //sumar los cargos adicionales de la orden
    let chargesTotal = 0;
    if (dataInvoice.charges && dataInvoice.charges.length > 0) {
      dataInvoice.charges.forEach((charge) => {
        chargesTotal += charge.total;
      });
    }
    order.status = configInvoiceOrder.id;
    order.subTotal = subTotal;
    order.chargesTotal = chargesTotal;
    order.total = subTotal + chargesTotal;
    order.updatedAt = new Date();
    ...
    await upsertSyncQueue(
      {
        ...
        recordId: order.id,
        action: "update",
        userId: dataInvoice.userId,
      },
      transaction,
    );

    // 5.2 Cambiar estado de todos los items a INVOICED
    for (const item of orderDetails) {
      item.status = configInvoiceItem.id;
      item.updatedAt = new Date();
      await item.save({ transaction });
    ...
    const invNumber = await generateNextInvoiceNumber(transaction);
    ...
    const invoiceItemsData = orderDetails.map((item: OrderDetail) => ({
      ...
      discount: dataInvoice.discounts,
    ...
    //insertar los cargos adicionales como items de la factura
    if (dataInvoice.charges && dataInvoice.charges.length > 0) {
      const chargesData = dataInvoice.charges.map(
        (charge: ChargeInvoiceDTO) => ({
          invoiceId: invoice.id,
          paramId: charge.paramChargeId,
          amount: charge.total,
          taxRate: 0,
          taxAmount: 0,
          status: STATUS_ENUM.ACTIVE,
      ...
      const chargesItems = await InvoicesSurcharges.bulkCreate(chargesData, {
        transaction,
        returning: true,
      });

      // insertar en SyncQueue cada cargo adicional de la factura
      for (const chargeItem of chargesItems) {
        await upsertSyncQueue(
          {
            tableName: "invoices_surcharges",
            recordId: chargeItem.id,
            action: "insert",
            userId: dataInvoice.userId,
          },
          transaction,
    ...
    const _order = await getOrder(dataInvoice.orderId);
