...
export async function syncPullAll(): Promise<RESPONSE> {
  try {
    ...
    await syncClientData(client.idClient);
    await syncFromCloud(client.idClient);

    return NEW_RESPONSE(0, "Sincronización pull completada");
  } catch (error) {
    return NEW_RESPONSE(-1, "Error en sync pull: " + error);
  }
}
