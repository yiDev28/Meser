...
const CardOrder: React.FC<CardOrderProps> = ({ order }) => {
  ...
  return (
    <div
      className={`relative flex flex-col bg-clip-border rounded-xl  text-neutral-dark shadow-md px-2 gap-2 border-5 border-neutral-light `}
      style={{
        //backgroundColor: `${order.type.colorLabel}`,
        background: `linear-gradient(to bottom, ${order.type.colorLabel}, var(--color-background-light) )`,
      }}
    >
      {/* Contenedor principal para usar Flexbox y empujar el botón hacia abajo */}
      <div className="flex flex-col flex-grow">
        <div className="flex justify-end mx-6">
          <div className=" bg-clip-border rounded-xl overflow-hidden text-white shadow-lg absolute -mt-6 grid h-15 w-15 place-items-center">
            <div
              style={{
        ...
        <div className="p-2 text-left">
          <div className=" text-background-light  flex justify-start items-center gap-2">
          ...
          <div>
            {order.type.paramType === "DELIVERY" && order.guestCustomer && (
              <>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold text-neutral-dark">
                    {order.guestCustomer.address} {order.guestCustomer.codCity}
                  </span>
                </div>
              </>
            )}
        ...
        <div className="border-t border-neutral-gray/40  ">
          <ul className="border border-neutral-gray/30 rounded-xl overflow-hidden w-full mt-2">
            {order.items.map((item: OrderItemDTO) => {
              const IconItems = iconItemOrder[item.itemStatus.paramStatus].icon;
              return (
                <li
                  key={item.id}
                  className="px-3 py-1 bg-background-light  border-b last:border-none border-neutral-gray"
                >
                  <div className="flex justify-between items-center">
                    <div className="flex items-center justify-center">
                      ...
                      <div>
                        <h4>{item.menuItem.product.name}</h4>
