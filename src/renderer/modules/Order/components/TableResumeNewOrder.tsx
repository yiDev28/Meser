...
const TableResumeNewOrder: React.FC<TableResumeNewOrderProps> = ({
  ...
  return (
    <div className="rounded-lg border border-neutral-gray/50 shadow-xl p-2  gap-2 flex flex-col">
      ...
      <table className="w-full  bg-neutral-light rounded-lg">
        ...
        <tbody>
          {payload.items.map((i) => {
            ...
            return (
              <tr key={i.menuItemId} className="border-t">
                <td className="p-2">{product?.product.name}
                    {i.notes && <p className="text-neutral-dark/70 text-sm">{i.notes}</p>}
                </td>
                <td className="p-2 font-semibold text-center">{i.quantity}</td>
          ...
          <tr className="border-t font-bold">
            ...
            <td className="p-2 text-secondary font-bold text-lg">
              {formatCurrency(
