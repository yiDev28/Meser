...
interface ButtonProps {
  ...
  mode?: "light"| "primary"|"primary_light"| "quaternary"| "quaternary_light"| "success"| "warning"| "danger"| "info"|"danger_light";
  size?: "xs"| "sm" | "md" | "lg" | "xl";
  label?: string;
...
export function ButtonActions({
  ...
  const typeClasses: Record<string, string> = {
    light:
      "bg-background text-neutral-gray outline hover:bg-neutral-gray hover:text-neutral-light",
    primary: "bg-primary text-neutral-light hover:bg-primary/80",
    primary_light: "bg-neutral-light border border-primary text-primary hover:text-neutral-light hover:bg-primary/80",
    ...
    danger_light: "bg-error-light border border-error text-error hover:bg-error/85 hover:text-neutral-light",
  ...
  const sizeClasses: Record<string, { button: string; icon: string }> = {
    xs: { button: "py-0.5 px-0.5 text-sm", icon: "w-3 h-3" },
  ...
  return (
    <button
      className={`cursor-pointer rounded-md flex items-center gap-2 disabled:opacity-30 disabled:cursor-default ${typeClasses[mode]} ${sizeClasses[size].button}`}
      onClick={onClick}
      type={type}
      disabled={disabled}
    >
